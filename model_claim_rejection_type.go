...
// ClaimRejectionType `INVALID_CUSTOMER_DEMOGRAPHIC_INFORMATION` - Customer demographic information is invalid.  `INVALID_INSURANCE_INFORMATION` - Customer insurance information is invalid.  `INVALID_DAYS_SUPPLY` - Days supply value is invalid.  `HARD_FAILURE` - Customer will not receive coverage, offer cash or transfer out.  `NEEDS_PRIOR_AUTHORIZATION` - Prescription requires Prior Authorization.  `REFILL TOO SOON` - Claim was rejected for refill too soon  `MANUAL_ADJUDICATION` - Claim Rejection might be addressed with manual adjudication and does not fall under another rejection\"
type ClaimRejectionType string

// List of ClaimRejectionType
const (
	INVALID_CUSTOMER_DEMOGRAPHIC_INFORMATION ClaimRejectionType = "INVALID_CUSTOMER_DEMOGRAPHIC_INFORMATION"
	INVALID_INSURANCE_INFORMATION            ClaimRejectionType = "INVALID_INSURANCE_INFORMATION"
	INVALID_DAYS_SUPPLY                      ClaimRejectionType = "INVALID_DAYS_SUPPLY"
	HARD_FAILURE                             ClaimRejectionType = "HARD_FAILURE"
	NEEDS_PRIOR_AUTHORIZATION                ClaimRejectionType = "NEEDS_PRIOR_AUTHORIZATION"
	REFILL_TOO_SOON                          ClaimRejectionType = "REFILL_TOO_SOON"
	MANUAL_ADJUDICATION                      ClaimRejectionType = "MANUAL_ADJUDICATION"
)

// All allowed values of ClaimRejectionType enum
var AllowedClaimRejectionTypeEnumValues = []ClaimRejectionType{
...
func (v *ClaimRejectionType) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := ClaimRejectionType(value)
	for _, existing := range AllowedClaimRejectionTypeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid ClaimRejectionType", value)
}

// NewClaimRejectionTypeFromValue returns a pointer to a valid ClaimRejectionType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewClaimRejectionTypeFromValue(v string) (*ClaimRejectionType, error) {
	ev := ClaimRejectionType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ClaimRejectionType: valid values are %v", v, AllowedClaimRejectionTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ClaimRejectionType) IsValid() bool {
	for _, existing := range AllowedClaimRejectionTypeEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to ClaimRejectionType value
func (v ClaimRejectionType) Ptr() *ClaimRejectionType {
	return &v
}

type NullableClaimRejectionType struct {
	value *ClaimRejectionType
	isSet bool
}

func (v NullableClaimRejectionType) Get() *ClaimRejectionType {
	return v.value
}

func (v *NullableClaimRejectionType) Set(val *ClaimRejectionType) {
	v.value = val
	v.isSet = true
}

func (v NullableClaimRejectionType) IsSet() bool {
	return v.isSet
}

func (v *NullableClaimRejectionType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableClaimRejectionType(val *ClaimRejectionType) *NullableClaimRejectionType {
	return &NullableClaimRejectionType{value: val, isSet: true}
}

func (v NullableClaimRejectionType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableClaimRejectionType) UnmarshalJSON(src []byte) error {
	v.isSet = true
