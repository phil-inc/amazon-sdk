...
// AllergyCodeType Enum representing the type of allergy code. `SNOMEDCT` - SNOMED CT code is one of a suite of designated standards for use in U.S. Federal Government systems to support electronic exchange of clinical health information.  `RXNORM` - RxNorm, a standardized nomenclature for clinical drugs, is produced and maintained by the National Library of Medicine.
type AllergyCodeType string

// List of AllergyCodeType
const (
	SNOMEDCT AllergyCodeType = "SNOMEDCT"
	RXNORM AllergyCodeType = "RXNORM"
)

// All allowed values of AllergyCodeType enum
var AllowedAllergyCodeTypeEnumValues = []AllergyCodeType{
	"SNOMEDCT",
	"RXNORM",
}

func (v *AllergyCodeType) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := AllergyCodeType(value)
	for _, existing := range AllowedAllergyCodeTypeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid AllergyCodeType", value)
}

// NewAllergyCodeTypeFromValue returns a pointer to a valid AllergyCodeType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewAllergyCodeTypeFromValue(v string) (*AllergyCodeType, error) {
	ev := AllergyCodeType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for AllergyCodeType: valid values are %v", v, AllowedAllergyCodeTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v AllergyCodeType) IsValid() bool {
	for _, existing := range AllowedAllergyCodeTypeEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to AllergyCodeType value
func (v AllergyCodeType) Ptr() *AllergyCodeType {
	return &v
}

type NullableAllergyCodeType struct {
	value *AllergyCodeType
	isSet bool
}

func (v NullableAllergyCodeType) Get() *AllergyCodeType {
	return v.value
}

func (v *NullableAllergyCodeType) Set(val *AllergyCodeType) {
	v.value = val
	v.isSet = true
}

func (v NullableAllergyCodeType) IsSet() bool {
	return v.isSet
}

func (v *NullableAllergyCodeType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAllergyCodeType(val *AllergyCodeType) *NullableAllergyCodeType {
	return &NullableAllergyCodeType{value: val, isSet: true}
}

func (v NullableAllergyCodeType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAllergyCodeType) UnmarshalJSON(src []byte) error {
	v.isSet = true
