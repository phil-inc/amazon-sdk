...
// ExistingMedicalConditionsDescriptor Enum representing the descriptor for the patient's existing medical conditions.  `KNOWN_MEDICAL_CONDITIONS_INCLUDED` - Patient's existing medical conditions are included in the existingMedicalConditions field.  `PATIENT_HAS_NO_KNOWN_MEDICAL_CONDITIONS` - Patient does not have any known medical conditions.  `NOT_AVAILABLE` - This should be set when: 1. Information is not available in the requested format. 2. Information has not been collected from the patient. 3. Patient did not provide the information. 4. Information cannot be provided due to some other reason.
type ExistingMedicalConditionsDescriptor string

// List of ExistingMedicalConditionsDescriptor
const (
...
var AllowedExistingMedicalConditionsDescriptorEnumValues = []ExistingMedicalConditionsDescriptor{
	"KNOWN_MEDICAL_CONDITIONS_INCLUDED",
	"PATIENT_HAS_NO_KNOWN_MEDICAL_CONDITIONS",
	"NOT_AVAILABLE",
}

func (v *ExistingMedicalConditionsDescriptor) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := ExistingMedicalConditionsDescriptor(value)
	for _, existing := range AllowedExistingMedicalConditionsDescriptorEnumValues {
	...
	return fmt.Errorf("%+v is not a valid ExistingMedicalConditionsDescriptor", value)
}

// NewExistingMedicalConditionsDescriptorFromValue returns a pointer to a valid ExistingMedicalConditionsDescriptor
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewExistingMedicalConditionsDescriptorFromValue(v string) (*ExistingMedicalConditionsDescriptor, error) {
	ev := ExistingMedicalConditionsDescriptor(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ExistingMedicalConditionsDescriptor: valid values are %v", v, AllowedExistingMedicalConditionsDescriptorEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ExistingMedicalConditionsDescriptor) IsValid() bool {
	for _, existing := range AllowedExistingMedicalConditionsDescriptorEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to ExistingMedicalConditionsDescriptor value
func (v ExistingMedicalConditionsDescriptor) Ptr() *ExistingMedicalConditionsDescriptor {
	return &v
}

type NullableExistingMedicalConditionsDescriptor struct {
	value *ExistingMedicalConditionsDescriptor
	isSet bool
}

func (v NullableExistingMedicalConditionsDescriptor) Get() *ExistingMedicalConditionsDescriptor {
	return v.value
}

func (v *NullableExistingMedicalConditionsDescriptor) Set(val *ExistingMedicalConditionsDescriptor) {
	v.value = val
	v.isSet = true
}

func (v NullableExistingMedicalConditionsDescriptor) IsSet() bool {
	return v.isSet
}

func (v *NullableExistingMedicalConditionsDescriptor) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableExistingMedicalConditionsDescriptor(val *ExistingMedicalConditionsDescriptor) *NullableExistingMedicalConditionsDescriptor {
	return &NullableExistingMedicalConditionsDescriptor{value: val, isSet: true}
}

func (v NullableExistingMedicalConditionsDescriptor) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableExistingMedicalConditionsDescriptor) UnmarshalJSON(src []byte) error {
	v.isSet = true
