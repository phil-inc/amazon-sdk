...
// PregnancyStatus Enum representing the pregnancy status of the patient.  `PREGNANT` - The patient is pregnant.  `NOT_PREGNANT` - The patient is not pregnant.  `NOT_AVAILABLE` - This should be set when: 1. Information is not available in the requested format. 2. Information has not been collected from the patient. 3. Patient did not provide the information. 4. Information cannot be provided due to some other reason.
type PregnancyStatus string

// List of PregnancyStatus
const (
	PREGNANT PregnancyStatus = "PREGNANT"
	NOT_PREGNANT PregnancyStatus = "NOT_PREGNANT"
	NOT_AVAILABLE PregnancyStatus = "NOT_AVAILABLE"
)

// All allowed values of PregnancyStatus enum
var AllowedPregnancyStatusEnumValues = []PregnancyStatus{
	"PREGNANT",
	"NOT_PREGNANT",
	"NOT_AVAILABLE",
}

func (v *PregnancyStatus) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PregnancyStatus(value)
	for _, existing := range AllowedPregnancyStatusEnumValues {
	...
	return fmt.Errorf("%+v is not a valid PregnancyStatus", value)
}

// NewPregnancyStatusFromValue returns a pointer to a valid PregnancyStatus
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPregnancyStatusFromValue(v string) (*PregnancyStatus, error) {
	ev := PregnancyStatus(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PregnancyStatus: valid values are %v", v, AllowedPregnancyStatusEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PregnancyStatus) IsValid() bool {
	for _, existing := range AllowedPregnancyStatusEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to PregnancyStatus value
func (v PregnancyStatus) Ptr() *PregnancyStatus {
	return &v
}

type NullablePregnancyStatus struct {
	value *PregnancyStatus
	isSet bool
}

func (v NullablePregnancyStatus) Get() *PregnancyStatus {
	return v.value
}

func (v *NullablePregnancyStatus) Set(val *PregnancyStatus) {
	v.value = val
	v.isSet = true
}

func (v NullablePregnancyStatus) IsSet() bool {
	return v.isSet
}

func (v *NullablePregnancyStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePregnancyStatus(val *PregnancyStatus) *NullablePregnancyStatus {
	return &NullablePregnancyStatus{value: val, isSet: true}
}

func (v NullablePregnancyStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePregnancyStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
