...
type MedicationPrescribed struct {
	...
	DaysSupply *float64 `json:"daysSupply,omitempty"`
	// The date on which the prescriber wrote the prescription. Accepts two formats, with DateTime in UTC preferred: 1. Date and time in UTC timezone: 'YYYY-MM-DDTHH:MM:SSZ' 2. Simple date: 'YYYY-MM-DD'  Examples: - '2025-07-30T14:30:00Z' (preferred format) - '2025-07-30' (date only)
	WrittenDate string `json:"writtenDate" validate:"regexp=^\\\\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])(?:T([01]\\\\d|2[0-3]):([0-5]\\\\d):([0-5]\\\\d)(?:\\\\.\\\\d{3,9})?Z)?$"`
	// The number of refills allowed for this prescription excluding the first fill.
	...
	// The date on which the user is intended to start the prescription. Accepts two formats, with DateTime in UTC preferred: 1. Date and time in UTC timezone: 'YYYY-MM-DDTHH:MM:SSZ' 2. Simple date: 'YYYY-MM-DD'  Examples: - '2025-07-30T14:30:00Z' (preferred format) - '2025-07-30' (date only)
	EffectiveDate *string `json:"effectiveDate,omitempty" validate:"regexp=^\\\\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])(?:T([01]\\\\d|2[0-3]):([0-5]\\\\d):([0-5]\\\\d)(?:\\\\.\\\\d{3,9})?Z)?$"`
	// The date on which the prescription expires. Accepts two formats, with DateTime in UTC preferred: 1. Date and time in UTC timezone: 'YYYY-MM-DDTHH:MM:SSZ' 2. Simple date: 'YYYY-MM-DD'  Examples: - '2025-07-30T14:30:00Z' (preferred format) - '2025-07-30' (date only)
	ExpirationDate *string      `json:"expirationDate,omitempty" validate:"regexp=^\\\\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])(?:T([01]\\\\d|2[0-3]):([0-5]\\\\d):([0-5]\\\\d)(?:\\\\.\\\\d{3,9})?Z)?$"`
...
// SetDaysSupply gets a reference to the given float64 and assigns it to the DaysSupply field.
