...
// ExistingMedicationsDescriptor Enum representing the descriptor for the patient's existing medications.  `EXISTING_MEDICATIONS_INCLUDED` - Patient's existing medications are included in the existingMedications field.  `PATIENT_IS_NOT_ON_ANY_MEDICATIONS` - Patient is currently not on any medications.  `NOT_AVAILABLE` - This should be set when: 1. Information is not available in the requested format. 2. Information has not been collected from the patient. 3. Patient did not provide the information. 4. Information cannot be provided due to some other reason.
type ExistingMedicationsDescriptor string

// List of ExistingMedicationsDescriptor
const (
	EXISTING_MEDICATIONS_INCLUDED     ExistingMedicationsDescriptor = "EXISTING_MEDICATIONS_INCLUDED"
	PATIENT_IS_NOT_ON_ANY_MEDICATIONS ExistingMedicationsDescriptor = "PATIENT_IS_NOT_ON_ANY_MEDICATIONS"
	NOT_AVAILABLE_3                   ExistingMedicationsDescriptor = "NOT_AVAILABLE"
)

// All allowed values of ExistingMedicationsDescriptor enum
var AllowedExistingMedicationsDescriptorEnumValues = []ExistingMedicationsDescriptor{
	"EXISTING_MEDICATIONS_INCLUDED",
	"PATIENT_IS_NOT_ON_ANY_MEDICATIONS",
	"NOT_AVAILABLE",
}

func (v *ExistingMedicationsDescriptor) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := ExistingMedicationsDescriptor(value)
	for _, existing := range AllowedExistingMedicationsDescriptorEnumValues {
	...
	return fmt.Errorf("%+v is not a valid ExistingMedicationsDescriptor", value)
}

// NewExistingMedicationsDescriptorFromValue returns a pointer to a valid ExistingMedicationsDescriptor
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewExistingMedicationsDescriptorFromValue(v string) (*ExistingMedicationsDescriptor, error) {
	ev := ExistingMedicationsDescriptor(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ExistingMedicationsDescriptor: valid values are %v", v, AllowedExistingMedicationsDescriptorEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ExistingMedicationsDescriptor) IsValid() bool {
	for _, existing := range AllowedExistingMedicationsDescriptorEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to ExistingMedicationsDescriptor value
func (v ExistingMedicationsDescriptor) Ptr() *ExistingMedicationsDescriptor {
	return &v
}

type NullableExistingMedicationsDescriptor struct {
	value *ExistingMedicationsDescriptor
	isSet bool
}

func (v NullableExistingMedicationsDescriptor) Get() *ExistingMedicationsDescriptor {
	return v.value
}

func (v *NullableExistingMedicationsDescriptor) Set(val *ExistingMedicationsDescriptor) {
	v.value = val
	v.isSet = true
}

func (v NullableExistingMedicationsDescriptor) IsSet() bool {
	return v.isSet
}

func (v *NullableExistingMedicationsDescriptor) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableExistingMedicationsDescriptor(val *ExistingMedicationsDescriptor) *NullableExistingMedicationsDescriptor {
	return &NullableExistingMedicationsDescriptor{value: val, isSet: true}
}

func (v NullableExistingMedicationsDescriptor) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableExistingMedicationsDescriptor) UnmarshalJSON(src []byte) error {
	v.isSet = true
