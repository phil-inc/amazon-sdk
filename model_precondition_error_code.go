...
// PreconditionErrorCode Enum representing the different error codes for precondition validation failures.  `FEATURE_NOT_ENABLED` - The requested feature is not enabled.  `PRESCRIPTION_DISCONTINUED` - The prescription has been discontinued.  `ORDER_COMPLETE` - The order is already complete.  `PRESCRIPTION_ID_DOES_NOT_EXIST` - The provided prescription ID does not exist.  `PRESCRIPTION_TRANSFER_ALREADY_INITIATED` - The prescription transfer has already been initiated.  `PRESCRIPTION_TRANSFER_ALREADY_COMPLETE` - The prescription transfer is already complete.  `NON_CONTRACTED_INSURANCE_PLAN` - The insurance plan is not contracted by Amazon Pharmacy.  `INVALID_INSURANCE_PLAN_INPUT` - The insurance plan provided is invalid.  `UNKNOWN_INSURANCE_PLAN_INPUT` - The insurance plan provided is unknown to Amazon Pharmacy.  `INCORRECT_PRESCRIPTION_TRANSFERRED_IN_DETAILS` - The prescription transferred in details are missing or incorrect.  `GOVERNMENT_INSURANCE_ON_FILE` - Patient already has an active government insurance listed with Amazon Pharmacy
type PreconditionErrorCode string

// List of PreconditionErrorCode
const (
	FEATURE_NOT_ENABLED PreconditionErrorCode = "FEATURE_NOT_ENABLED"
	PRESCRIPTION_DISCONTINUED_1 PreconditionErrorCode = "PRESCRIPTION_DISCONTINUED"
	ORDER_COMPLETE PreconditionErrorCode = "ORDER_COMPLETE"
	PRESCRIPTION_ID_DOES_NOT_EXIST PreconditionErrorCode = "PRESCRIPTION_ID_DOES_NOT_EXIST"
	PRESCRIPTION_TRANSFER_ALREADY_INITIATED PreconditionErrorCode = "PRESCRIPTION_TRANSFER_ALREADY_INITIATED"
	PRESCRIPTION_TRANSFER_ALREADY_COMPLETE PreconditionErrorCode = "PRESCRIPTION_TRANSFER_ALREADY_COMPLETE"
	NON_CONTRACTED_INSURANCE_PLAN PreconditionErrorCode = "NON_CONTRACTED_INSURANCE_PLAN"
	INVALID_INSURANCE_PLAN_INPUT PreconditionErrorCode = "INVALID_INSURANCE_PLAN_INPUT"
	UNKNOWN_INSURANCE_PLAN_INPUT PreconditionErrorCode = "UNKNOWN_INSURANCE_PLAN_INPUT"
	INCORRECT_PRESCRIPTION_TRANSFERRED_IN_DETAILS PreconditionErrorCode = "INCORRECT_PRESCRIPTION_TRANSFERRED_IN_DETAILS"
	GOVERNMENT_INSURANCE_ON_FILE PreconditionErrorCode = "GOVERNMENT_INSURANCE_ON_FILE"
)

// All allowed values of PreconditionErrorCode enum
var AllowedPreconditionErrorCodeEnumValues = []PreconditionErrorCode{
	"FEATURE_NOT_ENABLED",
...
func (v *PreconditionErrorCode) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PreconditionErrorCode(value)
	for _, existing := range AllowedPreconditionErrorCodeEnumValues {
	...
	return fmt.Errorf("%+v is not a valid PreconditionErrorCode", value)
}

// NewPreconditionErrorCodeFromValue returns a pointer to a valid PreconditionErrorCode
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPreconditionErrorCodeFromValue(v string) (*PreconditionErrorCode, error) {
	ev := PreconditionErrorCode(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PreconditionErrorCode: valid values are %v", v, AllowedPreconditionErrorCodeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PreconditionErrorCode) IsValid() bool {
	for _, existing := range AllowedPreconditionErrorCodeEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to PreconditionErrorCode value
func (v PreconditionErrorCode) Ptr() *PreconditionErrorCode {
	return &v
}
