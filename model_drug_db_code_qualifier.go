...
// DrugDbCodeQualifier Enum representing the Drug Qualifier  `E` - Truven/Micromedex Generic Formulation Code (GFC)  `G` - Truven/Micromedex Generic Master (GM)  `AF` - American Hospital Formulary Service (AHFS)  `FG` - First DataBank Clinical Formulation ID Sequence Number(GCN_SEQNO)  `FS` - First Databank Smartkey  `MC` - Multum Drug ID  `MD` - Medi Span's Drug Descriptor ID (DDID)  `MG` - Medi Span's Generic Product Identifier (GPI)  `MM` - Multum MMDC  `FL` - First DataBank Ingredient List ID (HICL_SEQNO)  `FM` - First DataBank Medication ID (FDB MedID)  `FN` - First DataBank Medication Name ID (FDB Med Name ID)  `FD` - First DataBank Routed Dosage Form ID (FDB Routed Dosage Form Med ID)  `GPK` - RxNorm Generic Package (GPCK)  `SCD` - RxNorm Semantic Clinical Drug (SCD)  `BPK` - RxNorm Branded Package (BPCK)  `SBD` - RxNorm Semantic Branded Drug (SBD)  `FI` - First DataBank Medication ID (FDB MedID)  `FR` - First DataBank Routed Medication ID (FDB Routed Med ID)  `GS` - Gold Standard Product Item Collection  `GMP` - Gold Standard Marketed Product Identifier (MPid)  `GPI` - Gold Standard Product Identifier (ProdID)  `GSP` - Gold Standard Specific Product Identifier (SPID)  `US` - U.S. Pharmacopoeia (USP)
type DrugDbCodeQualifier string

// List of DrugDbCodeQualifier
const (
	E   DrugDbCodeQualifier = "E"
	G   DrugDbCodeQualifier = "G"
	AF  DrugDbCodeQualifier = "AF"
	FG  DrugDbCodeQualifier = "FG"
	FS  DrugDbCodeQualifier = "FS"
	MC  DrugDbCodeQualifier = "MC"
	MD  DrugDbCodeQualifier = "MD"
	MG  DrugDbCodeQualifier = "MG"
	MM  DrugDbCodeQualifier = "MM"
	FL  DrugDbCodeQualifier = "FL"
	FM  DrugDbCodeQualifier = "FM"
	FN  DrugDbCodeQualifier = "FN"
	FD  DrugDbCodeQualifier = "FD"
	GPK DrugDbCodeQualifier = "GPK"
	SCD DrugDbCodeQualifier = "SCD"
	BPK DrugDbCodeQualifier = "BPK"
	SBD DrugDbCodeQualifier = "SBD"
	FI  DrugDbCodeQualifier = "FI"
	FR  DrugDbCodeQualifier = "FR"
	GS  DrugDbCodeQualifier = "GS"
	GMP DrugDbCodeQualifier = "GMP"
	GPI DrugDbCodeQualifier = "GPI"
	GSP DrugDbCodeQualifier = "GSP"
	US  DrugDbCodeQualifier = "US"
)

// All allowed values of DrugDbCodeQualifier enum
var AllowedDrugDbCodeQualifierEnumValues = []DrugDbCodeQualifier{
	"E",
	"G",
	"AF",
	"FG",
...
func (v *DrugDbCodeQualifier) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := DrugDbCodeQualifier(value)
	for _, existing := range AllowedDrugDbCodeQualifierEnumValues {
	...
	return fmt.Errorf("%+v is not a valid DrugDbCodeQualifier", value)
}

// NewDrugDbCodeQualifierFromValue returns a pointer to a valid DrugDbCodeQualifier
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewDrugDbCodeQualifierFromValue(v string) (*DrugDbCodeQualifier, error) {
	ev := DrugDbCodeQualifier(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for DrugDbCodeQualifier: valid values are %v", v, AllowedDrugDbCodeQualifierEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v DrugDbCodeQualifier) IsValid() bool {
	for _, existing := range AllowedDrugDbCodeQualifierEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to DrugDbCodeQualifier value
func (v DrugDbCodeQualifier) Ptr() *DrugDbCodeQualifier {
	return &v
}

type NullableDrugDbCodeQualifier struct {
	value *DrugDbCodeQualifier
	isSet bool
}

func (v NullableDrugDbCodeQualifier) Get() *DrugDbCodeQualifier {
	return v.value
}

func (v *NullableDrugDbCodeQualifier) Set(val *DrugDbCodeQualifier) {
	v.value = val
	v.isSet = true
}

func (v NullableDrugDbCodeQualifier) IsSet() bool {
	return v.isSet
}

func (v *NullableDrugDbCodeQualifier) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDrugDbCodeQualifier(val *DrugDbCodeQualifier) *NullableDrugDbCodeQualifier {
	return &NullableDrugDbCodeQualifier{value: val, isSet: true}
}

func (v NullableDrugDbCodeQualifier) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDrugDbCodeQualifier) UnmarshalJSON(src []byte) error {
	v.isSet = true
