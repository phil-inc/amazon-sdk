...
// PrescriptionStatus Enum representing the prescription status that will be surfaced to partners.  `PRESCRIPTION_RECEIVED` - The prescription has been received after the API call has been accepted.  `PRESCRIPTION_CREATED` - The prescription has been created but not active. This state is before data entry.  `PRESCRIPTION_FAILED` - The prescription has failed. Please correct any issues and send with a new ID. After received, we check for failures such as missing data, incorrect formatting, etc. The prescription can be resubmitted under this state.  `PRESCRIPTION_PRE_PV1` - The prescription is in the pre-PV1 state, waiting to be typed.  `PRESCRIPTION_PV1` - The prescription is in the PV1 state, waiting to be verified by a pharmacist after data-entry completion.  `PRESCRIPTION_PV1_FOLLOW_UP` - Prescription requires additional information before data entry can be completed to resolve missing or unclear information. This is applied during Data Entry, requires additional intervention by pharmacist to patient or prescriber.  `PRESCRIPTION_ACTIVE` - The prescription is active post Pharmacist verification and approval.  `PRESCRIPTION_DISCONTINUED` - The prescription has been discontinued and no longer available for usage Pharmacist has determined that we cannot fulfill this prescription. This could be due to multiple reasons, one of them being prescription being transferred.  `PRESCRIPTION_INVALID` - The prescription is invalid due to insufficient information and cannot be typed.
type PrescriptionStatus string

// List of PrescriptionStatus
const (
	PRESCRIPTION_RECEIVED      PrescriptionStatus = "PRESCRIPTION_RECEIVED"
	PRESCRIPTION_CREATED       PrescriptionStatus = "PRESCRIPTION_CREATED"
	PRESCRIPTION_FAILED        PrescriptionStatus = "PRESCRIPTION_FAILED"
	PRESCRIPTION_PRE_PV1       PrescriptionStatus = "PRESCRIPTION_PRE_PV1"
	PRESCRIPTION_PV1           PrescriptionStatus = "PRESCRIPTION_PV1"
	PRESCRIPTION_PV1_FOLLOW_UP PrescriptionStatus = "PRESCRIPTION_PV1_FOLLOW_UP"
	PRESCRIPTION_ACTIVE        PrescriptionStatus = "PRESCRIPTION_ACTIVE"
	PRESCRIPTION_DISCONTINUED  PrescriptionStatus = "PRESCRIPTION_DISCONTINUED"
	PRESCRIPTION_INVALID       PrescriptionStatus = "PRESCRIPTION_INVALID"
)

// All allowed values of PrescriptionStatus enum
var AllowedPrescriptionStatusEnumValues = []PrescriptionStatus{
...
func (v *PrescriptionStatus) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PrescriptionStatus(value)
	for _, existing := range AllowedPrescriptionStatusEnumValues {
	...
	return fmt.Errorf("%+v is not a valid PrescriptionStatus", value)
}

// NewPrescriptionStatusFromValue returns a pointer to a valid PrescriptionStatus
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPrescriptionStatusFromValue(v string) (*PrescriptionStatus, error) {
	ev := PrescriptionStatus(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PrescriptionStatus: valid values are %v", v, AllowedPrescriptionStatusEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PrescriptionStatus) IsValid() bool {
	for _, existing := range AllowedPrescriptionStatusEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to PrescriptionStatus value
func (v PrescriptionStatus) Ptr() *PrescriptionStatus {
	return &v
}

type NullablePrescriptionStatus struct {
	value *PrescriptionStatus
	isSet bool
}

func (v NullablePrescriptionStatus) Get() *PrescriptionStatus {
	return v.value
}

func (v *NullablePrescriptionStatus) Set(val *PrescriptionStatus) {
	v.value = val
	v.isSet = true
}

func (v NullablePrescriptionStatus) IsSet() bool {
	return v.isSet
}

func (v *NullablePrescriptionStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePrescriptionStatus(val *PrescriptionStatus) *NullablePrescriptionStatus {
	return &NullablePrescriptionStatus{value: val, isSet: true}
}

func (v NullablePrescriptionStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePrescriptionStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
