...
// OrderStatus Enum representing the order status.  `ORDER_CREATED` - The order for the medication has been placed on Amazon.  `ORDER_CANCELLED` - The order was cancelled by the partner using the CancelPartnerOrder API.  `ORDER_FAILED` - The order has failed due to the reason specified in the OrderStatusReasonCode.  `ORDER_ON_HOLD` - The payment for the order was soft-declined, and the payment processing will be retried after some time.  `ORDER_PROCESSING` - The order is being actively worked on by the fulfillment team.  `ORDER_RECEIVED` - The order request has been accepted by Amazon.  `ORDER_SHIPPED` - The order has been shipped and is on its way to the customer.
...
const (
	...
	ORDER_FAILED     OrderStatus = "ORDER_FAILED"
	ORDER_ON_HOLD    OrderStatus = "ORDER_ON_HOLD"
	ORDER_PROCESSING OrderStatus = "ORDER_PROCESSING"
