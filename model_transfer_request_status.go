...
// TransferRequestStatus Enum representing the status of a transfer request.  `TRANSFER_INITIATED` - Prescription transfer has been initialized by Amazon Pharmacy and is under internal processing.  `TRANSFER_COMPLETE` - Prescription transfer is legally complete from Amazon Pharmacy.  `TRANSFER_DENIED` - Prescription transfer has been denied by Pharmacists.
type TransferRequestStatus string

// List of TransferRequestStatus
const (
	TRANSFER_INITIATED TransferRequestStatus = "TRANSFER_INITIATED"
	TRANSFER_COMPLETE  TransferRequestStatus = "TRANSFER_COMPLETE"
	TRANSFER_DENIED    TransferRequestStatus = "TRANSFER_DENIED"
)

// All allowed values of TransferRequestStatus enum
var AllowedTransferRequestStatusEnumValues = []TransferRequestStatus{
	"TRANSFER_INITIATED",
	"TRANSFER_COMPLETE",
	"TRANSFER_DENIED",
}

func (v *TransferRequestStatus) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := TransferRequestStatus(value)
	for _, existing := range AllowedTransferRequestStatusEnumValues {
	...
	return fmt.Errorf("%+v is not a valid TransferRequestStatus", value)
}

// NewTransferRequestStatusFromValue returns a pointer to a valid TransferRequestStatus
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewTransferRequestStatusFromValue(v string) (*TransferRequestStatus, error) {
	ev := TransferRequestStatus(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for TransferRequestStatus: valid values are %v", v, AllowedTransferRequestStatusEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v TransferRequestStatus) IsValid() bool {
	for _, existing := range AllowedTransferRequestStatusEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to TransferRequestStatus value
func (v TransferRequestStatus) Ptr() *TransferRequestStatus {
	return &v
}

type NullableTransferRequestStatus struct {
	value *TransferRequestStatus
	isSet bool
}

func (v NullableTransferRequestStatus) Get() *TransferRequestStatus {
	return v.value
}

func (v *NullableTransferRequestStatus) Set(val *TransferRequestStatus) {
	v.value = val
	v.isSet = true
}

func (v NullableTransferRequestStatus) IsSet() bool {
	return v.isSet
}

func (v *NullableTransferRequestStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTransferRequestStatus(val *TransferRequestStatus) *NullableTransferRequestStatus {
	return &NullableTransferRequestStatus{value: val, isSet: true}
}

func (v NullableTransferRequestStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTransferRequestStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
