...
// ErrorField Structure contains the Error field and error message specific to the field.
type ErrorField struct {
	// Indicates the field name responsible for the error
	Field string `json:"field" validate:"regexp=^\\\\s*\\\\S.*$"`
	// Message describing why the field was invalid
	Message *string `json:"message,omitempty" validate:"regexp=^\\\\s*\\\\S.*$"`
}

type _ErrorField ErrorField
...
func (o *ErrorField) UnmarshalJSON(data []byte) (err error) {
	...
	varErrorField := _ErrorField{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varErrorField)

	if err != nil {
		return err
	}

	*o = ErrorField(varErrorField)
