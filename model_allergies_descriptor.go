...
// AllergiesDescriptor Enum representing the descriptor for the patient's allergies.  `KNOWN_ALLERGIES_INCLUDED` - Patient's allergies are included in the allergies field.  `PATIENT_HAS_NO_KNOWN_ALLERGIES` - Patient does not have any known allergies.  `NOT_AVAILABLE` - This should be set when: 1. Information is not available in the requested format. 2. Information has not been collected from the patient. 3. Patient did not provide the information. 4. Information cannot be provided due to some other reason.
type AllergiesDescriptor string

// List of AllergiesDescriptor
const (
...
var AllowedAllergiesDescriptorEnumValues = []AllergiesDescriptor{
	"KNOWN_ALLERGIES_INCLUDED",
	"PATIENT_HAS_NO_KNOWN_ALLERGIES",
	"NOT_AVAILABLE",
}

func (v *AllergiesDescriptor) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := AllergiesDescriptor(value)
	for _, existing := range AllowedAllergiesDescriptorEnumValues {
	...
	return fmt.Errorf("%+v is not a valid AllergiesDescriptor", value)
}

// NewAllergiesDescriptorFromValue returns a pointer to a valid AllergiesDescriptor
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewAllergiesDescriptorFromValue(v string) (*AllergiesDescriptor, error) {
	ev := AllergiesDescriptor(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for AllergiesDescriptor: valid values are %v", v, AllowedAllergiesDescriptorEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v AllergiesDescriptor) IsValid() bool {
	for _, existing := range AllowedAllergiesDescriptorEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to AllergiesDescriptor value
func (v AllergiesDescriptor) Ptr() *AllergiesDescriptor {
	return &v
}

type NullableAllergiesDescriptor struct {
	value *AllergiesDescriptor
	isSet bool
}

func (v NullableAllergiesDescriptor) Get() *AllergiesDescriptor {
	return v.value
}

func (v *NullableAllergiesDescriptor) Set(val *AllergiesDescriptor) {
	v.value = val
	v.isSet = true
}

func (v NullableAllergiesDescriptor) IsSet() bool {
	return v.isSet
}

func (v *NullableAllergiesDescriptor) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAllergiesDescriptor(val *AllergiesDescriptor) *NullableAllergiesDescriptor {
	return &NullableAllergiesDescriptor{value: val, isSet: true}
}

func (v NullableAllergiesDescriptor) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAllergiesDescriptor) UnmarshalJSON(src []byte) error {
	v.isSet = true
