...
// PatientStatus Enum representing the external patient status surfaced to partners.  `PATIENT_RECEIVED` - Represents that the Patient data has been received by Amazon Pharmacy and is pending further processing.  `PATIENT_SUCCESS` - Represents that the Patient data has been successfully saved by Amazon Pharmacy.  `PATIENT_FAILED` - Represents that the Patient data processing has failed due to the provided status reason code.
type PatientStatus string

// List of PatientStatus
const (
	PATIENT_RECEIVED PatientStatus = "PATIENT_RECEIVED"
	PATIENT_SUCCESS PatientStatus = "PATIENT_SUCCESS"
	PATIENT_FAILED PatientStatus = "PATIENT_FAILED"
)

// All allowed values of PatientStatus enum
var AllowedPatientStatusEnumValues = []PatientStatus{
	"PATIENT_RECEIVED",
	"PATIENT_SUCCESS",
	"PATIENT_FAILED",
}

func (v *PatientStatus) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PatientStatus(value)
	for _, existing := range AllowedPatientStatusEnumValues {
	...
	return fmt.Errorf("%+v is not a valid PatientStatus", value)
}

// NewPatientStatusFromValue returns a pointer to a valid PatientStatus
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPatientStatusFromValue(v string) (*PatientStatus, error) {
	ev := PatientStatus(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PatientStatus: valid values are %v", v, AllowedPatientStatusEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PatientStatus) IsValid() bool {
	for _, existing := range AllowedPatientStatusEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to PatientStatus value
func (v PatientStatus) Ptr() *PatientStatus {
	return &v
}

type NullablePatientStatus struct {
	value *PatientStatus
	isSet bool
}

func (v NullablePatientStatus) Get() *PatientStatus {
	return v.value
}

func (v *NullablePatientStatus) Set(val *PatientStatus) {
	v.value = val
	v.isSet = true
}

func (v NullablePatientStatus) IsSet() bool {
	return v.isSet
}

func (v *NullablePatientStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePatientStatus(val *PatientStatus) *NullablePatientStatus {
	return &NullablePatientStatus{value: val, isSet: true}
}

func (v NullablePatientStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePatientStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
